...
# Sumber proxy gratis. Persentase = hasil ukur sendiri ke chat.b.ai/api/auth/csrf
# (bukan klaim penyedia; yang penting jumlah proxy HIDUP, bukan total kandidat).
#   hproxy-socks5     12-20%  <- 1.158 kandidat -> ~140-230 hidup, stok terbesar
#   monosans           9-27%  <- 789 kandidat, skema lengkap, ada socks
#   roosterkid SOCKS5    37%  <- kecil (5-8) tapi segar, commit per jam
#   databay-fast         5%  <- 384 kandidat, sudah disaring "fast" oleh penyedia
#   databay              3%  <- 6.078 kandidat
#   roosterkid SOCKS4   20%
#   proxifly http+socks5 13%  <- rapuh, 3 dari 4 mati saat dites ulang
#   proxyscrape v4      10%  <- API asli, kena rate limit, cooldown panjang
#   speedx http          1%  <- cadangan
#   shiftytr             0%  <- DIBUANG: commit terakhir 2023-08-11, mati semua
PROXY_SOURCES: list[dict] = [
    {"name": "hproxy-socks5", "scheme": "socks5", "cooldown": 60,
     "url": "https://raw.githubusercontent.com/hproxy-com/free-proxy-list/main/socks5.txt"},
    {"name": "hproxy-socks4", "scheme": "socks4", "cooldown": 60,
     "url": "https://raw.githubusercontent.com/hproxy-com/free-proxy-list/main/socks4.txt"},
    ...
    {"name": "databay-fast", "scheme": None, "cooldown": 60,
     "url": "https://databay.com/api/v1/proxy-list?speed=fast&format=txt"},
    {"name": "databay", "scheme": None, "cooldown": 90,
...
def scrape_proxies(sources: list[dict] | None = None,
    ...
    Sumber diambil PARALEL: 11 sumber × ~1s kalau berurutan jadi ~11s, paralel
    jadi ~1-2s. `workers` membatasi berapa sumber yang diambil bersamaan.
    """
    sources = sources if sources is not None else PROXY_SOURCES
    now = time.monotonic()
    todo = [s for s in sources
            if cache is None or now >= cache.source_cooldown_until(s["name"])]
    out: list[str] = []
    if not todo:
        return out
